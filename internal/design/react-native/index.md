...
- The player **features** in
  ...
  are written against `HTMLMediaElement` — they call
  `addEventListener('play')`, read `media.paused` / `media.readyState`, and
  reference `HTMLMediaElement.HAVE_FUTURE_DATA`. There is no `<video>` element
  in React Native; the media surface is a native player behind a JSI/bridge
  boundary.
...
- **Platform-only capabilities** that don't exist on the web — e.g. the
  `<BackgroundPlayer.Provider>` and the persistent `backgroundSession`.
...
The single persistent background player is a *separate* provider bound to the
app-owned session store (see
[Two providers over a shared core](#two-providers-over-a-shared-core)):

```tsx
import { BackgroundPlayer } from '@videojs/react-native';

<BackgroundPlayer.Provider>   {/* binds the singleton persistent session */}
  <Video />                   {/* attachable surface; absent in pure background */}
  <Controls />
</BackgroundPlayer.Provider>;
```
...
| Provider | `<Player.Provider>` / `<BackgroundPlayer.Provider>` | source, store, player-wide capabilities (`loop`, autoplay) |
...
component-scoped) and `<BackgroundPlayer.Provider>` (the singleton persistent
...
BackgroundPlayer.Provider → usePlayerProviderCore({ store: backgroundSession.store })   // binds the persistent store, singleton
```
...
**Surface hand-off is the provider's job.** Moving the surface between screens
is the existing media-registration flow (`setMedia` / `setContainer`) operating
against a persistent store: a screen's provider attaches its surface on connect
and detaches on disconnect, but — because the store is external and the engine
lives in the service — detaching the surface does **not** stop playback. So the
glitch-prone `AVPlayerLayer` re-point / `setVideoSurface` swap is bounded to
navigation events and handled by the same connect/disconnect machinery, not a
new mechanism.
...
session than as `BackgroundPlayer`.
...
- **Feature reuse mechanism.** Do the adapter-shared features stay literally
  the same code (generalized over the `Media` contract from
  [`media.md`](../media.md)), or are they re-authored per platform sharing only
  state shapes? Leaning toward generalizing over the contract — see
...
- **Persistent session naming: "background" vs. capability-neutral.**
  `<BackgroundPlayer.Provider>` reads as background-audio-specific, but the same
  persistent single-owner session also hosts [remote playback](#future-work-remote-playback-cast--airplay)
